...
Para verificar o desempenho da solução desenvolvida, foi empregado um algoritmo de simulação envolvendo 300 terminais de clientes, distribuídos em 100 terminais para cada servidor. Os terminais realizaram simultaneamente a tentativa de comprar a mesma rota, uma passagem de A até D, sendo esta uma rota mista onde, o trecho de A até B estava no servidor A, o trecho B a C estava no servidor B e o trecho de C a D estava no servidor C. Nesta compra eram realizadas todas as etapas até a finalização de uma compra. Haviam 150 vagas disponíveis, sendo assim apenas metade dos terminais deveriam conseguir. Ao executar o algoritmo foi notado que de fato só metade dos terminais conseguiram comprar a passagem. Contudo, não houve uma distribuição equitativa entre os clientes, resultando em uma maior quantidade de passagens para alguns terminais conectados a determinadas companhias em comparação com outros.
